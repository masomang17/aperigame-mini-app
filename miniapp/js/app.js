...
const BASE_API_URL = 'https://script.google.com/macros/s/AKfycbwvQaQhM3cGxzmDgz0CTsebkzuF86-vjfXtbQnCj9LrtRqZNA-ovfn5kmjuRIHEk8w8vg/exec';
const IMAGE_BASE_URL = 'https://raw.githubusercontent.com/masomang17/aperigame-mini-app/main/miniapp/images/';

// --- TRADUZIONI ---
const translations = {
    it: {
        quiz: "Quiz", tip: "Consigli", profile: "Profilo", shop: "Premi",
        play: "Gioca Ora", user: "Utente", noStars: "0", prizeReq: "Riscatta",
        prizeCost: "Costo:", notEnoughStars: "Non hai abbastanza stelle per questo premio.",
        prizeSuccess: "Premio '{prizeTitle}' richiesto con successo!", quizPrizeText: "Premio per il quiz: ",
        quizExit: "Esci dal quiz", quizScore: "Hai totalizzato", quizPoints: "punti su",
        quizTimeUp: "Tempo scaduto! Punteggio 0 per questa domanda.", quizNext: "Domanda successiva",
        quizFinish: "Fine quiz!", loadingQuiz: "Caricamento quiz...",
        quizLoadError: "Errore nel caricamento del quiz.", quizDataMissing: "Dati quiz non disponibili.",
        answerRecorded: "Risposta registrata. I risultati verranno pubblicati più tardi!",
    },
...
let quizState = {
    currentIndex: 0, score: 0, timer: null, timeLeft: 0,
    timePerQuestion: 15, answeringAllowed: true, currentQuizSheet: null
};

// --- INIZIALIZZAZIONE ---
document.addEventListener("DOMContentLoaded", () => {
    // Inizializza dati utente Telegram
    ...
    // Carica dati iniziali e renderizza l'app
    loadInitialData();
});
...
function renderShop() {
    ...
    container.innerHTML = "";
    sendDataToScript({ action: 'viewShop', userId: telegramUserId, username: username }); // Logga la visita
...
async function loadQuizData(quizSheet, buttonElement) {
    ...
    try {
        ...
        rawData.forEach(item => {
            ...
            if (qText === "title") quizConfig.titles[quizSheet][lang] = item.answer_1;
            else if (qText !== "prize") {
...
function showQuizQuestion() {
    ...
    if (quizState.currentIndex >= questions.length) return showQuizResult();
...
function handleAnswer(selectedIndex) {
    ...
    if (q.correctIndex !== -1) { // Quiz con risposta giusta/sbagliata
        if (isCorrect) {
            quizState.score += q.points || 0;
            document.querySelectorAll("#quiz-answers button")[selectedIndex].classList.add('btn-correct');
        } else {
            document.querySelectorAll("#quiz-answers button")[selectedIndex].classList.add('btn-wrong');
            if (q.correctIndex >= 0) document.querySelectorAll("#quiz-answers button")[q.correctIndex].classList.add('btn-correct');
        }
    } else { // Sondaggio 1X2
        alert(translations[currentLang].answerRecorded);
        document.querySelectorAll("#quiz-answers button")[selectedIndex].classList.add('btn-selected-1x2');
    }
...
function showQuizResult(exitedEarly = false) {
    ...
    const containerId = quizState.currentQuizSheet === 'quiz' ? 'quiz-content' : 'tip-content';
    const container = document.getElementById(containerId);
    let message = `${translations[currentLang].quizScore} ${quizState.score} ${translations[currentLang].quizPoints} ${questions.length}.`;
    if (exitedEarly) message += `<br>${translations[currentLang].quizExit}`;
    container.innerHTML = `<div class="quiz-container"><h3>${translations[currentLang].quizFinish}</h3><p>${message}</p><button id="return-btn" class="play-btn">${translations[currentLang].quizExit}</button></div>`;
