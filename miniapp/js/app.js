...
const BASE_API_URL = 'https://script.google.com/macros/s/AKfycbz263cFtmATaZqWq2SWcSnb2a_TOW7sKAzyC-MavWV_IVNYUoc47JG18TuBSuKJJO6tVg/exec'; // Assicurati sia il tuo ultimo URL di deploy
const IMAGE_BASE_URL = 'https://raw.githubusercontent.com/masomang17/aperigame-mini-app/main/miniapp/images/';

// --- TRADUZIONI ---
const translations = {
    it: {
        quiz: "Quiz", pronostici: "Pronostici", profile: "Profilo", negozio: "Negozio", play: "Gioca Ora",
        user: "Utente", noStars: "0", prizeReq: "Acquista", prizeCost: "Costo:",
        notEnoughStars: "Non hai abbastanza stelle.", prizeSuccess: "Acquisto completato!",
...
function renderNegozioPage() {
    ...
    negozioItems.forEach(item => {
        ...
        card.innerHTML = `
            ${item.url_immagine ? `<img src="${IMAGE_BASE_URL}${item.url_immagine}" class="prize-image">` : ''}
            <div class="prize-title">${item.titolo}</div>
            <p class="prize-description">${item.descrizione || ''}</p>
            <p class="prize-cost">⭐ ${item.prezzo_stars}</p>
            <button class="play-btn">${translations[currentLang].prizeReq}</button>`;
...
async function purchaseItem(item, button) {
    ...
    const result = await sendDataToScript({
        action: 'purchaseItem', userId: telegramUserId, username: username,
        firstName: telegramFirstName, lastName: telegramLastName,
        id_prodotto: item.id_prodotto, titolo: item.titolo, prezzo_stars: item.prezzo_stars
    });
    if (result?.status === 'success') {
        userProfile.stars = result.data.newStars;
        renderProfile();
        alert(translations[currentLang].prizeSuccess.replace('{prizeTitle}', item.titolo));
    } else {
        alert(result.message || "Errore durante l'acquisto.");
    }
...
async function loadQuizData(quizSheet, buttonElement) {
    ...
    try {
        ...
        quizConfig = { ...quizConfig, ...result.data };
    } catch (error) {
...
function handleAnswer(selectedIndex) {
    ...
    if (isCorrect) {
        ...
        if (q.indice_corretto >= 0) buttons[q.indice_corretta].classList.add('btn-correct');
    }
    setTimeout(() => {
...
async function endQuizSequence() {
    ...
    if (result.status === 'success') {
        ...
        loadInitialData(); // Ricarica i dati del profilo per aggiornare stelle/punti
    }
    else {
        alert("Errore salvataggio risultato: " + (result.message || 'Errore'));
        showSection(quizState.currentQuizSheet); // Torna alla pagina del quiz
    }
}
